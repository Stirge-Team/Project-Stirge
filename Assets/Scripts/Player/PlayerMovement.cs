...
namespace Stirge.Player
{
    ...
    public class PlayerMovement : MonoBehaviour
    {
        ...
        void FixedUpdate()
        {
            ...
            Vector3 cameraForward2d = new Vector3(
                m_cameraTransform.forward.x,
                0f,
                m_cameraTransform.forward.z
            ).normalized;
        ...
        public bool OnJump()
        {
            ...
            if (IsGrounded)
            {
                ...
                m_motor.ApplyForce(
                    transform.up * Mathf.Sqrt(2 * m_jumpHeight * -Physics.gravity.y),
                    ForceMode.Impulse
                );
        ...
        public void OnDrawGizmosSelected()
        {
            ...
            switch (IsGrounded)
            {
                ...
                case false:
                    Gizmos.DrawSphere(
                        transform.position + transform.up * (m_jumpHeight - m_lastCheckedHeight),
                        0.5f
                    );
                    break;
            }
            ...
            if (m_motor)
                Gizmos.DrawRay(
                    transform.position,
                    transform.up * Mathf.Clamp(m_motor._verticalVelocity, -Mathf.Infinity, 0)
                );
