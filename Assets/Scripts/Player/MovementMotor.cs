...
public class MovementMotor : MonoBehaviour
{
  ...
  public void ClampHorizontalVelocity(float min, float max)
  {
    if(enabled)
    {
      ...
      {
        m_rb.linearVelocity -= new Vector3(_horizontalVelocity.x + min * _horizontalDirection.x, 0, _horizontalVelocity.z + min * _horizontalDirection.z);
      }
      if(max < _horizontalSpeed)
      {
        m_rb.linearVelocity -= new Vector3(_horizontalVelocity.x - max * _horizontalDirection.x, 0, _horizontalVelocity.z - max * _horizontalDirection.z);
      }
    }
  }
