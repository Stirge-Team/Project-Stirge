...
namespace Stirge.Camera
{
    public class TrackingCamera : MonoBehaviour
    {
        ...
        private struct cameraSettings
        {
            ...
            [Tooltip(
                "How much the distance between the desiered and target camera angle should affect the pivot speed."
            )]
        ...
        [
            SerializeField,
            Tooltip(
                "The primary target for the camera - used for position and as the default object to look at"
            )
        ]
        ...
        [
            SerializeField,
            Tooltip(
                "The secondary targets for the camera - used for the math on where the camera points. The 0th index of the array is for the lockon target"
            )
        ]
        ...
        [
            SerializeField,
            Tooltip(
                "How strong the distance scaling should be during group situations (multiplicative)"
            )
        ]
        ...
        [
            SerializeField,
            Tooltip(
                "How biased the camera should be to enemies in range while in the combat camera mode (should it look closer to the enemies or the player?)"
            ),
            Range(0, 1)
        ]
        ...
        [
            SerializeField,
            Tooltip(
                "Uses the primary target's forward for finding the targets instead of the camera's"
            )
        ]
        ...
        [
            SerializeField,
            Tooltip("Should the lock on release when the locked on target is out of range?")
        ]
        ...
        [
            SerializeField,
            Tooltip(
                "Should (if none are in range) targets outside of range be used as lock on targets."
            )
        ]
        ...
        void Update()
        {
            ...
            switch (m_camState)
            {
                case CameraStates.Explore:
                    if (m_secondaryTargets.Length > 0)
                        ...
                        {
                            if (
                                (m_primaryTarget.position - targ.position).magnitude
                                <= m_combatRangeCutoff
                            )
                            {
                    ...
                    float primaryDistanceFromCamera = (
                        new Vector3(
                            m_primaryTarget.position.x - transform.position.x,
                            0,
                            m_primaryTarget.position.z - transform.position.z
                        )
                    ).magnitude;
                    ...
                    m_cameraDesiredPosition =
                        ...
                        + new Vector3(
                            ...
                            Mathf.Sin(m_viewAngle) * m_exploreSettings._relativePosition.x
                        ); //Same as the other
                    ...
                    m_cameraDesiredLookPoint =
                        m_primaryTarget.position
                        + (
                            m_primaryTarget.position
                            - new Vector3(
                                m_cameraDesiredPosition.x,
                                m_primaryTarget.position.y,
                                m_cameraDesiredPosition.z
                            )
                ...
                case CameraStates.Combat:
                    ...
                    if (m_secondaryTargets.Length == 0)
                    {
                        ...
                        Debug.LogWarning(
                            "There are no combat targets for the camera to track... take your schizo pills (or check your code)"
                        );
                        break;
                    }
                    ...
                    if (m_cleanTargetList.Count == 0)
                    {
                        ChangeState(CameraStates.Explore);
                        Debug.LogWarning(
                            "There are no combat targets in range... player ran away!"
                        );
                        break;
                    }
                    ...
                    m_cameraDesiredLookPoint =
                        m_primaryTarget.position + clumpDistanceToPrimary * m_clumpBias;

                    //Find the futhest target from the centre of combat
                    float furthestTargetDistance = (
                        m_cameraDesiredLookPoint - m_primaryTarget.position
                    ).magnitude;
                    foreach (var targ in m_cleanTargetList)
                    {
                    ...
                    float distanceScaler = Mathf.Clamp(
                        furthestTargetDistance * m_groupDistanceScaling,
                        1,
                        Mathf.Infinity
                    );
                    ...
                    m_cameraDesiredPosition =
                        m_cameraDesiredLookPoint
                        + new Vector3(
                            Mathf.Cos(m_viewAngle)
                                * m_combatSettings._relativePosition.x
                                * distanceScaler, //Move back from that centre point given the current angle, distance between the targets & any added space
                            m_combatSettings._relativePosition.y, //just moves it up
                            Mathf.Sin(m_viewAngle)
                                * m_combatSettings._relativePosition.x
                                * distanceScaler
                        ); //Same as the other
                    ...
                    if (m_canAutoRotate)
                    {
                        ...
                        directionToLookPoint = new Vector3(
                            directionToLookPoint.x,
                            0,
                            directionToLookPoint.z
                        ).normalized;

                        SetDesiredAngle(
                            Mathf.Atan2(directionToLookPoint.z, directionToLookPoint.x)
                                - Mathf.PI / 2f,
                            false
                        );
                    }
                ...
                case CameraStates.LockOn:
                    ...
                    Vector3 betweenTargetPosition =
                        (m_lockedOnTarget.position - m_primaryTarget.position) / 2;
                    float distanceScalerForLockOn = Mathf.Clamp(
                        betweenTargetPosition.magnitude * m_groupDistanceScaling,
                        1,
                        Mathf.Infinity
                    );
                    ...
                    if (m_canAutoRotate)
                    {
                        SetDesiredAngle(
                            Mathf.Atan2(
                                m_primaryTarget.position.z - m_lockedOnTarget.position.z,
                                m_primaryTarget.position.x - m_lockedOnTarget.position.x
                            )
                    ...
                    m_cameraDesiredPosition =
                        ...
                        + new Vector3(
                            Mathf.Cos(m_viewAngle)
                                * m_lockOnSettings._relativePosition.x
                                * distanceScalerForLockOn, //Move back from that centre point given the current angle, distance between the targets & any added space
                            m_currentSettings._relativePosition.y, //just moves it up
                            Mathf.Sin(m_viewAngle)
                                * m_lockOnSettings._relativePosition.x
                                * distanceScalerForLockOn
                        );
            ...
            transform.position = Vector3.Lerp(
                transform.position,
                m_cameraDesiredPosition,
                Time.deltaTime * m_currentSettings._movementSpeed
            );
            transform.rotation = Quaternion.Lerp(
                transform.rotation,
                Quaternion.LookRotation(m_cameraDesiredLookPoint - transform.position),
        ...
        public void OnDrawGizmosSelected()
        {
            ...
            Gizmos.DrawRay(
                m_primaryTarget.position,
                new Vector3(
                    Mathf.Cos(m_desiredAngle) * m_currentSettings._relativePosition.x,
                    m_currentSettings._relativePosition.y,
                    Mathf.Sin(m_desiredAngle) * m_currentSettings._relativePosition.x
                )
            );
            ...
            Gizmos.DrawSphere(
                transform.position
                    + transform.forward * (m_cameraDesiredLookPoint - transform.position).magnitude,
                0.25f
            );
            Gizmos.DrawRay(
                m_primaryTarget.position,
                new Vector3(
                    Mathf.Cos(m_viewAngle) * m_currentSettings._relativePosition.x,
                    m_currentSettings._relativePosition.y,
                    Mathf.Sin(m_viewAngle) * m_currentSettings._relativePosition.x
                )
            );
        ...
        private void LockOnStart()
        {
            ...
            while (closestAngle > 180)
            {
                ...
                for (int i = 0; i < m_secondaryTargets.Length; i++)
                {
                    ...
                    targDists[i] =
                        new Vector3(
                            m_secondaryTargets[i].position.x,
                            0,
                            m_secondaryTargets[i].position.z
                    ...
                    float newAngle = Vector3.Angle(
                        targDists[i].normalized,
                        m_usePrimaryForward ? m_primaryTarget.forward : transform.forward
                    );
        ...
        private Transform[] VerifyTargets()
        {
            ...
            targetList.AddRange(m_secondaryTargets);
            for (int i = 0; i < m_secondaryTargets.Length; i++)
            {
                if (m_secondaryTargets[i] == null)
                {
                    RemoveTarget(i);
                    i--;
                }
            }
            return m_secondaryTargets;
        }
    }
}
