...
namespace Stirge.Combat
{
    ...
    public abstract class CombatEntity : MonoBehaviour
    {
        ...
        private IEnumerator SpeedMove(SpeedMoveNode node)
        {
            ...
            while (!arrived)
            {
                Vector3 target = Vector3.MoveTowards(GetPosition(), endPosition, speed * Time.deltaTime);
                m_rb.MovePosition(target);

                // if arrived at end position
                if (m_rb.position == endPosition)
                {
                    arrived = true;
                    break;
                }
        ...
        private IEnumerator AccelerateMove(AccelerateMoveNode node)
        {
            ...
            while (!arrived)
            {
                ...
                Vector3 target = Vector3.MoveTowards(GetPosition(), endPosition, currentSpeed * Time.deltaTime);
                m_rb.MovePosition(target);
